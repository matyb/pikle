...
class ProcedureBrowse extends JFrame implements ActionListener,
		TreeSelectionListener {

	private static final long serialVersionUID = 2023899873915967998L;

	public ProcedureBrowse(MyInternalFrame mif, Connection connection,
			String userID) {
		super("Procedure Browser");
	...
	private void setupTree(DatabaseMetaData metadata) throws SQLException {
		String[] tableTypes = { "PROCEDURE" }; // We want only tables
	...
	public void valueChanged(TreeSelectionEvent e) {
		...
		for (int j = 0; j < paths.length; j++) {
			...
			case 2: // We have a table selected
				...
				message = "Complete " + tableParam + " procedure displayed";
				break;
		...
		try {
			tableModel.setResultSet(statement.executeQuery(mif
					...
					.getDatabaseDialect()
					.getProcedureInfoSQLString(tableParam,
